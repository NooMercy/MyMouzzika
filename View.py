...
class View:
    def __init__(self, top=None):
        ...
        self.song_scale = ttk.Scale(top, from_=0, to=1.0)
        self.song_scale.place(relx=0.247, rely=0.932, relwidth=0.147
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                ...
                self.removeFavourite.config(command=self.remove_songs_from_favourites)
            else:
        ...
        self.song_scale.config(from_=0, to=100,command=self.seek_music)
        ...
        self.deleteSongsFromPlaylistButton.config(command=self.delete_song_from_playlist_button)
        self.my_thread=None
    ...
    def forwarding_song(self,curr_sec):
        move_to = self.song_scale.get()
        value = int((float(curr_sec) * self.song_length) / 100)
        self.my_player.play_seek_song(value)

    def seek_music(self, value):
        if self.isPlaying:
            self.seek_value = value
            progress_percent = self.curr_sec / float(self.song_length) * 100
    ...
    def add_song_to_favourites(self):
        ...
        try:
            if len(fav_song_index_tuple) == 0:
                raise NoSongSelectedError ("Please select a song before adding to favourite")
    ...
    def show_timer(self,total_sec):
        ...
        self.song_scale.set(0)
        self.curr_sec = present_val
        self.songProgress.stop()

        while present_val <= total_sec:
            print("Show Timer")
            if self.difference > 2 or self.difference < 0:
                if self.isPaused == False:
                    present_val =int((float(self.song_scale.get()) * self.song_length) / 100)
                    self.curr_sec = present_val
                    #self.stopThread = True
                    self.forwarding_song(present_val)
                    time.sleep(0.5)
            ...
            if self.isPaused == True:
                ...
                self.song_scale.set(progress_percent)
                present_val+=1
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            if result.find("No Songs Prsent")!=-1:
                messagebox.showinfo("Favourites Empty!","No Songs Prsent in your Favourite")
                return
            ...
            mycolor="#"+red+green+blue
            self.playList.config(fg=mycolor)
            messagebox.showinfo("Favourites Loaded!", "Songs Loaded Successfully!")
        except NoSongSelectedError as ex1:
            messagebox.showerror("Error",ex1)
