...
class Model:
    def __init__(self):
        ...
        self.cur=None
        try:
            self.conn = connect("advjavabatch/admin@127.0.1/xe")
    ...
    def load_songs_from_favourites(self):
        ...
        if songs_present:
            ...
            return "No Songs Prsent in your Favourites"
    def remove_song_from_favourites(self,song_name):
        self.cur.execute("delete from myfavourites where song_name=:1",(song_name,))
        n=self.cur.rowcount
        if n==0:
            return "The Selected Song is Not Present in the Playlist"
        else:
            #self.song_dict.pop(song_name)
            self.conn.commit()
            return "Song Successfully Removed From the Playlist"
